...
class Hive{
 ...
 render(){
  ...
  const lines = this.svg.selectAll(".axis")
    ...
    // d3.json(this.json).then((nodes) => {
      ...
      this.nodes.forEach(source=>{
        source.imports.forEach(targetName=>{//遍历nodes下每一个imports的属性
          ...
          this.links.push({source: source.source, target: target.target})
        })
      })
   ...
   if(this.color == true && this.colors !== undefined || this.colors !== null){
    this.nodesByType.forEach((d,i)=>{
      ...
      this.nodesByType.forEach(type=> {//遍历nodesByType
        var lastName = type[1][0].packageName, count = 0;
    ...
    this.radius.domain(d3.extent(this.nodes,d=>{return d.index}));

    
    
    // this.render(this.nodes,this.nodesByType,this.links)
    //绘制轴
  ...
  d3.json('./hiv.json').then((nodes) => {
    ...
    var  p = new Hive({
      ...
      json : './hiv.json',
      father:div,
      styles:[{'stroke':'blue'},{'stroke-width':3+'px'}],
      color:true,
